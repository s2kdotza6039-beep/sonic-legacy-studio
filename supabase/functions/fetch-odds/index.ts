...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const results = await Promise.allSettled(
      requestedLeagues.map(async (sportKey) => {
        const url = `${ODDS_API_BASE}/sports/${sportKey}/odds/?apiKey=${API_KEY}&regions=uk&markets=${markets}&oddsFormat=decimal`;
        const response = await fetch(url);
        if (!response.ok) { console.warn(`Failed to fetch ${sportKey}: ${response.status}`); return { events: [], sportKey }; }
    ...
    for (const result of results) {
      ...
      const { events, sportKey } = result.value;
      for (const event of events) {
        ...
        allEvents.push({
          ...
          sport_key: sportKey, kickoff: kickoffStr, commence_time: event.commence_time,
