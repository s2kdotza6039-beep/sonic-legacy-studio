...
import { Brain, Eye, EyeOff, Plus, Save, Star, Trash2 } from "lucide-react";
      ...
      <div className="flex items-center justify-between border-b border-border px-4 py-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm uppercase tracking-widest font-bold">Knowledge Vault</h3>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1.05fr_1.4fr]">
        <div className="border-b lg:border-b-0 lg:border-r border-border p-4 space-y-3">
          <div className="flex items-center justify-between">
            <p className="text-[10px] uppercase tracking-widest text-muted-foreground">Knowledge Entries</p>
            <Badge variant="secondary" className="text-[10px]">{loading ? "Loading..." : `${entries.length} total`}</Badge>
          </div>
          <div className="space-y-2 max-h-[540px] overflow-auto pr-1">
            {entries.length === 0 && !loading ? (
              <div className="rounded-md border border-dashed border-border p-4 text-sm text-muted-foreground">
                No founder knowledge yet.
              </div>
            ) : (
              entries.map((entry) => {
