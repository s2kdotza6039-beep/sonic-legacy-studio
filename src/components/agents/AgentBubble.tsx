...
import { trackAgentEvent } from "@/lib/agentAnalytics";
...
const AgentBubble = ({ agent, autoGreet }: Props) => {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(
      () =>
        setPhase((p) => {
          if (p !== "button") return p;
          trackAgentEvent(agent.id, "greeting_shown");
          return "greeting";
        }),
      900,
    );
    return () => clearTimeout(t);
  }, [autoGreet, greetKey, agent.id]);
  ...
  // Move focus into the panel/greeting when it opens; return it to the launcher on close.
  useEffect(() => {
    if (phase === "chat") {
    ...
    if (phase === "greeting") {
      const t = setTimeout(
        () => greetRef.current?.querySelector<HTMLElement>(FOCUSABLE)?.focus(),
        60,
      );
      return () => clearTimeout(t);
    }
  ...
  const dismissGreeting = () => {
    ...
    trackAgentEvent(agent.id, "greeting_dismissed");
    setPhase("button");
  ...
  const savePosRef = useRef<() => void>();
  savePosRef.current = () => savePos(pos);

  // Keyboard: Escape closes, Tab cycles inside the panel (focus trap).
  ...
  const send = async (raw?: string, source: "input" | "chip" = "input") => {
    const text = (raw ?? input).trim();
    if (!text || isLoading) return;
    if (countWords(text) > MAX_WORDS) return;

    trackAgentEvent(agent.id, "send", source, { words: countWords(text) });
  ...
  const openChatWith = (prompt?: string, chipLabel?: string) => {
    try {
      sessionStorage.setItem(greetKey, "1");
    } catch {}
    if (chipLabel) trackAgentEvent(agent.id, "chip_click", chipLabel);
    setPhase("chat");
    if (prompt) void send(prompt, "chip");
  };
    ...
    <>
      ...
      {phase === "button" && (
        <button
          ref={launcherRef}
          onClick={() => {
            trackAgentEvent(agent.id, "button_open");
            openChatWith();
          }}
          aria-label={`Open chat with ${agent.name}, ${agent.role}`}
          aria-haspopup="dialog"
          className={`fixed bottom-6 ${anchor} z-50 group flex items-center gap-3 rounded-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background`}
        >
          <span className="relative w-14 h-14 shrink-0">
            <span aria-hidden="true" className="absolute inset-0 rounded-full bg-primary/25 animate-ping" />
            <span
              aria-hidden="true"
          ...
          <span aria-hidden="true" className="hidden sm:flex flex-col items-start bg-card/90 backdrop-blur border border-border px-3 py-1.5 rounded-full shadow-lg">
      ...
      {phase === "greeting" && (
        <div
          ref={greetRef}
          role="dialog"
          aria-label={`Greeting from ${agent.name}, ${agent.role}`}
          onKeyDown={(e) => {
        ...
        >
          <div className="border border-primary/40 bg-card/95 backdrop-blur rounded-lg shadow-2xl overflow-hidden">
            <div aria-hidden="true" className="h-1 w-full bg-gradient-to-r from-transparent via-primary to-transparent" />
            <div className="p-4">
              <div className="flex items-start justify-between gap-3">
                ...
                <button
                  ...
                  aria-label={`Minimise ${agent.name}'s greeting`}
                  className="p-1 text-muted-foreground hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary rounded"
                >
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {agent.chips.map((c) => (
                  <button
                    key={c.label}
                    onClick={() => openChatWith(c.prompt, c.label)}
                    aria-label={`Start a chat with ${agent.name}: ${c.label}`}
                    className="text-[11px] uppercase tracking-widest border border-primary/40 text-primary px-3 py-1.5 rounded-full hover:bg-primary hover:text-primary-foreground transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
                  >
                    {c.label}
                  </button>
                ))}
              ...
              <button
                onClick={() => {
                  trackAgentEvent(agent.id, "chat_open", "greeting_typing");
                  openChatWith();
                }}
                className="mt-4 w-full text-[10px] uppercase tracking-widest text-muted-foreground hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary rounded"
              >
      ...
      {phase === "chat" && (
        <div
          ...
          ref={panelRef}
          role="dialog"
          aria-label={`Chat with ${agent.name}, ${agent.role}`}
        ...
        >
          ...
          >
            <div className="flex items-center gap-2">
              {!expanded && <GripVertical size={14} aria-hidden="true" className="text-muted-foreground" />}
              {nameMark}
              <span className="text-[9px] uppercase tracking-widest text-muted-foreground">
                · {agent.role}
              </span>
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                aria-label={expanded ? `Shrink ${agent.name}'s chat panel` : `Expand ${agent.name}'s chat panel`}
                className="p-1 hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary rounded"
              >
          ...
          >
                ...
                <div className="flex flex-wrap gap-2">
                  {agent.chips.map((c) => (
                    <button
                      key={c.label}
                      onClick={() => {
                        trackAgentEvent(agent.id, "chip_click", c.label);
                        void send(c.prompt, "chip");
                      }}
                      aria-label={`Ask ${agent.name}: ${c.label}`}
                      className="text-[10px] uppercase tracking-widest border border-primary/40 text-primary px-2.5 py-1 rounded-full hover:bg-primary hover:text-primary-foreground transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
                    >
                      {c.label}
                    </button>
                  ))}
            ...
            {messages.map((m, i) => (
              <div key={i} className={`flex gap-2 ${m.role === "user" ? "justify-end" : ""}`}>
                ...
                >
                  <span className="sr-only">{m.role === "user" ? "You said:" : `${agent.name} said:`}</span>
                  {m.role === "assistant" ? (
          ...
          <div className="border-t border-border p-2 space-y-1">
            <div className="flex gap-2">
              <Textarea
                ...
                aria-label={`Write a message to ${agent.name} (maximum ${MAX_WORDS} words). Press Enter to send, Shift plus Enter for a new line.`}
                aria-invalid={overLimit}
                aria-describedby={`${agent.id}-word-count`}
